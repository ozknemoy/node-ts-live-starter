...
export class PrintExtraLaborContractDynamicBuilder {
  ...
  private makeHeader() {
    ...
    /*const header = new Paragraph()
      .center()
      .style('12')
      .addRun(new TextRun(`Дополнительное соглашение к Трудовому договору от ${HandleData.getRuDate(this.activeWorkplace.contractDate) || '____________'}\t№ ${this.activeWorkplace.contractNumber || '____'}`).bold().break());
