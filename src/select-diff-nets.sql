...
from (
         ...
         WHERE s.DOC_TYPE_ID = :DOC_TYPE_ID AND (s.SELLER_ID = 1 OR s.SELLER_ID IS NULL) AND (s.BUYER_ID = 1 OR s.BUYER_ID IS NULL) and (s.ui_group = 1 OR s.ui_group IS NULL)
     ) where Priority = 1